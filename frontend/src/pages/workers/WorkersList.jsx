...
function WorkersList() {
    ...
    const fetchWorkers = async (page = 0) => {
        setLoading(true);
        try {
            const response = await axios.get(`/api/worker/list?page=${page}&size=${itemsPerPage}`, {
    ...
    const handleSubmit = async (workerData) => {
        try {
            if (workerData.worker_id) {
                await axios.put(`/api/worker/update/${workerData.worker_id}`, workerData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            } else {
                await axios.post("/api/worker/create", workerData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            }
            ...
            buttons: [
                {
                    ...
                    onClick: async () => {
                        try {
                            const response = await axios.delete(`/api/worker/delete/${workerId}`, {
                                headers: { Authorization: `Bearer ${token}` }
                            });
