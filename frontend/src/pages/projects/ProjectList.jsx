...
function ProjectList() {
    ...
    const fetchProjects = async (page = 0) => {
        setLoading(true);
        try {
            const response = await axios.get(`/api/project/list?page=${page}&size=${itemsPerPage}`, {
    ...
    const handleSubmit = async (projectData) => {
        try {
            if (projectData.id) {
                await axios.put(`/api/project/update/${projectData.id}`, projectData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
                toast.success('✅ Proje başarıyla güncellendi!');
            } else {
                await axios.post("/api/project/create", projectData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
